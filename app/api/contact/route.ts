...
export async function POST(request: Request) {
    ...
    const transporter = nodemailer.createTransport({
      ...
      return new Response(JSON.stringify({ message: 'Thank you, we will be in touch soon!' }), {
        status: 200,
        headers: {
      ...
      return new Response(JSON.stringify({ message: 'Error submitting form, please try again', error: error.message }), {
        status: 500,
        headers: {
          'Content-Type': 'application/json',
        },
